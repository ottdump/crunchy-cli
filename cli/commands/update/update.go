...
func update() error {
	...
	resp, err := utils.Client.Get("https://api.github.com/repos/ottdump/crunchy-cli/releases/latest")
	if err != nil {
		return err
	}
	...
	utils.Log.Info("A new version is available (%s): https://github.com/ottdump/crunchy-cli/releases/tag/v%s", releaseVersion, releaseVersion)
	...
	if updateInstallFlag {
		if runtime.GOARCH != "amd64" {
			return fmt.Errorf("invalid architecture found (%s), only amd64 is currently supported for automatic updating. "+
				"You have to update manually (https://github.com/ottdump/crunchy-cli)", runtime.GOARCH)
		}
		...
		default:
			return fmt.Errorf("invalid operation system found (%s), only linux, windows and darwin / macos are currently supported. "+
				"You have to update manually (https://github.com/ottdump/crunchy-cli", runtime.GOOS)
		}
		...
		resp, err := utils.Client.Get(fmt.Sprintf("https://github.com/ottdump/crunchy-cli/releases/download/v%s/%s", releaseVersion, downloadFile))
		if err != nil {
			return err
		}
		defer resp.Body.Close()
